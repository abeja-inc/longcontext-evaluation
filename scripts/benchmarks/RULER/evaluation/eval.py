...
import wandb
import yaml
from project_module.benchmark.ruler.evaluation import EvaluationConfig, EvaluationPipeline
...
def _wandb_log(summary_json: dict[str, Any], model_name: str, cfg_for_run: dict[str, Any]) -> None:
    ...
    group = cfg_for_run.get("group") or "RULER"
    ...
    artifact_name = f"{args.model_name}-ruler-eval".replace("/", "_")
    art = wandb.Artifact(
        name=artifact_name,
        type="evaluation",
    ...
    if pred_dir.exists():
        ...
        if jsonl_files:
            artifact_name = f"{args.model_name}-ruler-preds".replace("/", "_")
