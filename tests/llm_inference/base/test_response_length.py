...
class _TooLongChatGenerator(BaseGenerator):
    def _count_tokens(self, input: Prompt | Conversation, **kwargs: object) -> int:
        return 0
    ...
    def _chat(
        ...
        return [
            Response(input=conversation.prompt, outputs=[])
            for conversation in conversations
        ] + [Response(input="extra", outputs=[])]

    def _completion(self, *, prompts: list[Prompt], **kwargs: object) -> list[Response]:
        return [Response(input=prompt.prompt, outputs=[]) for prompt in prompts]


class _TooLongCompletionGenerator(BaseGenerator):
    def _count_tokens(self, input: Prompt | Conversation, **kwargs: object) -> int:
        return 0
...
def test_chat_raises_runtime_error_when__chat_returns_too_many_responses(
    logger: logging.Logger,
) -> None:
    generator = _TooLongChatGenerator(
        model_name="dummy",
        max_context_length=1,
        max_output_tokens=1,
        logger=logger,
    ...
    with pytest.raises(
        RuntimeError,
        match=r"\._chat must return same length as input: in=2 out=3",
    ):
        generator.chat(conversations=conversations)


def test_completion_raises_runtime_error_when__completion_returns_too_many_responses(
    logger: logging.Logger,
) -> None:
    generator = _TooLongCompletionGenerator(
        model_name="dummy",
        max_context_length=1,
        max_output_tokens=1,
        logger=logger,
    ...
    with pytest.raises(
        RuntimeError,
        match=r"\._completion must return same length as input: in=2 out=3",
    ):
        generator.completion(prompts=prompts)
