...
class MRCRResultsBuilder(ResultsBuilder):
    ...
    def build_results(
        ...
        *,
        ...
        tables = [
            Table(name="table/mrcr_output_table", rows=output_rows),
            Table(name="table/mrcr_score_by_sample_table", rows=score_table_rows),
            Table(name="metrics/mrcr_leaderboard", rows=leaderboard),
            Table(name="metrics/mrcr_score_by_context_length", rows=by_context),
        ]
        return Results(
            tables=tables,
            summary=summary,
